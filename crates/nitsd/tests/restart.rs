...
use nits_protocol::{
    Author, BuildInfo, ClientId, LifecycleNotice, ProtocolVersion, ReplayPosition, Request,
    Response, RpcError, Seq, Since, SubscribeScope, UpgradeId, UpgradeOperation, UpgradeProgress,
    UpgradeStage,
};
...
async fn unix_and_websocket_notices_bypass_filters_and_interrupt_waits_before_store_release() {
    ...
    unix.request(Request::Subscribe {
    ...
    let wait = tokio::spawn({
        let ws = Arc::clone(&ws);
        async move {
            ws.request(Request::ReplayEvents {
                scope: SubscribeScope::All,
                position: ReplayPosition::Follow { after: Seq::new(0) },
            })
            .await
        }
    });
    assert!(matches!(
        unix.request(Request::ListWorkspaces).await.unwrap(),
        Response::Workspaces { .. }
    ));
    ...
    let operation = daemon.prepare_restart(UpgradeOperation {
    ...
    assert_retirement_handshakes(&socket, &unix, &ws, &operation).await;
    ...
    assert!(
        ...
        "blocked accepted Core work still owns retirement"
    );
    ...
    drop(unix);
    drop(ws);
    tokio::time::timeout(Duration::from_secs(6), shutdown.cancelled())
        .await
        .unwrap();
...
async fn assert_retirement_handshakes(
    ...
    for client in [unix, ws] {
        let notice = tokio::time::timeout(Duration::from_secs(2), client.next_unsolicited())
            .await
            .unwrap()
            .unwrap();
        assert!(
            matches!(notice, Unsolicited::Lifecycle(LifecycleNotice::Restarting { operation: announced }) if announced.id == operation.id)
        );
    }
    ...
    assert!(
        matches!(current, ClientError::Rejected(RpcError::Restarting { operation_id }) if operation_id == operation.id)
    );
    let raw = tokio::net::UnixStream::connect(socket).await.unwrap();
    let old = Client::handshake(raw, identity(4), ProtocolVersion::new(0, 1, 0))
        .await
        .unwrap_err();
    assert!(matches!(
        old,
