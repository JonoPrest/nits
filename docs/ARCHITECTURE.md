...
- **Interrupted suggestion recovery**: the claim creates a brief absent-path interval. Original bytes and a recovery note are durably named in `nits-suggestion-*` under the worktree's Git metadata directory before installation; success cleans them. Failure restores the original only if the destination is still absent and its parent unchanged. An obstructed restore reports the recovery directory and keeps both `original` and `proposed`; a process interruption keeps the same evidence. Inspect `RECOVERY.txt`, the current checkout path and retained files before restoring anything, and never overwrite a newer checkout file. If installation completed, `original` is the previous content and the checkout already contains the proposed bytes. These guarantees cover descriptor-based traversal and path replacement; they are not a filesystem transaction against unrelated processes writing through previously opened descriptors. The success event is appended only after replacement finishes.
